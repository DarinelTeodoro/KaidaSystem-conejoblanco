...
function cargarTickets() {
    ...
    $.ajax({
        type: "GET",
        url: "../../controller/barra-comandas-pendientes.php",
        dataType: "json",
        timeout: 30000,
        success: function (todasLasComandas) {
            ...
            const tiempoMinimo = 0 * 1000; // 40 segundos en milisegundos
            ...
            const comandasFiltradas = todasLasComandas.filter(comanda => {
                ...
                const tiempoComanda = fechaComanda.getTime();
...
function marcarComandaCompletada(comandaId, button) {
    ...
    $.ajax({
        type: "POST",
        url: "../../controller/barra-completar-comanda.php",
...
function crearCardComanda(comanda, highlightKeys = new Set()) {
    ...
    // Solo mostrar el botón si la comanda NO está completada
...
document.addEventListener('click', e => {
    if (e.target.id === 'open_modal_outproducts') {
        ...
        $.ajax({
            type: "POST",
            url: "../../controller/barra-productos-agotados.php",
            data: {},
            success: function (response) {
...
document.addEventListener('click', e => {
    if (e.target.id === 'open_modal_history') {
        ...
        $.ajax({
            type: "POST",
            url: "../../controller/barra-history-comandas.php",
            data: {rol: 'barra'},
            success: function (response) {
