            ...
            <div class="body_alert">
                ...
                <div id="container-btn-acept">
                    <button type="button" href="home.php" class="btn_accept_alert me-1" onclick="hide_alert_2()"
                        style="background: #cacaca;">Nueva Comanda</button>
...
<script>
    ...
    $(document).ready(function () {
        ...
        // Limpiar inputs
        document.getElementById('mesa-pedido').value = 0;
        document.getElementById('nombre-delivery').value = '';
    ...
    document.addEventListener('change', (e) => {
        if (e.target?.id === 'type-delivery') {
            ...
            if (valueoption == 'mesa') {
                resetearSelectMesas();
                document.getElementById('cont_mesa_pedido').style.display = 'grid';
                document.getElementById('cont_nombre_delivery').style.display = 'none';
            } else if (valueoption == 'domicilio') {
                document.getElementById('cont_mesa_pedido').style.display = 'none';
                document.getElementById('cont_nombre_delivery').style.display = 'grid';
            }
        }
    });
  ...
  `;
    ...
    document.getElementById('form_new_comanda')
        ?.addEventListener('submit', function (e) {
            ...
            const tipo = document.getElementById('type-delivery')?.value;

            if (tipo === 'mesa') {
                const mesa = document.getElementById('mesa-pedido')?.value;
                if (mesa == 0) {
                    document.getElementById('btn-send-comanda').disabled = false;
                    show_alert("ALERTA", "Seleccione el número de mesa.");
                    return;
                }
            }

            if (tipo === 'domicilio') {
                const cliente = document.getElementById('nombre-delivery')?.value?.trim();
                if (!cliente) {
                    document.getElementById('btn-send-comanda').disabled = false;
                    show_alert("ALERTA", "Debes ingresar el nombre/domicilio del cliente.");
                    return;
                }
            }
            ...
            const mesa = document.getElementById('mesa-pedido')?.value || null;
            const cliente = document.getElementById('nombre-delivery')?.value || null;

            const payloadNew = {
                tipo,
                mesa: tipo === 'mesa' ? mesa : null,
                cliente: tipo === 'domicilio' ? cliente : null,
                items: ComandaStore.state.items
            };

            $.ajax({
                type: "POST",
                url: "../../controller/mesero-guardar-comanda.php",
                data: { payload: JSON.stringify(payloadNew) },
                success: function (response) {
                    ComandaStore.state.items = [];
                    ComandaStore.render();
                    document.getElementById('carrito_comanda')?.classList.remove('visible');
                    ...
                    hide_alert();
                    show_alert_2("EXITO", "Comanda creada.");
                },
