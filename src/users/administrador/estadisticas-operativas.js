...
function cargarDatosPorFecha(fecha) {
    ...
    $.ajax({
        url: '../../controller/estadisticas-ventas.php', // Ajusta esta URL
        type: 'POST',
        data: {dia: fecha},
        dataType: 'html',
        success: function (response) {
            $("#container-data-ventas").html(response);
        },
...
function mostrarCarga() {
    const contenedor = document.getElementById('container-data-ventas');
...
// Función para actualizar gráfica (ejemplo)
function actualizarGrafica(datos) {
    // Si usas Chart.js o similar
    if (window.graficaVentas) {
        window.graficaVentas.data.datasets[0].data = datos;
        window.graficaVentas.update();
    }
}
...
function pdf_ventas() {
    ...
    // Abre en una nueva ventana/pestaña
    window.open('../../controller/pdf-ventas.php?day=' + inputDia, '_blank');
}